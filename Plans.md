...
| 4.5 | grill-me 산출 경로 인자 지원 (L5) | SKILL.md에 대상 디렉토리 인자 규약 명시, 기본값은 현재 프로젝트 docs/ | grep -q '산출 경로' .claude/skills/grill-me/SKILL.md | - | cc:완료 | - |
| 4.6 | grill-me 비대화형 실행 호환 모드 (L5) | 무응답·headless 환경에서 질문마다 권장값으로 자동 확정 후 진행하는 대안 경로 명시 | grep -q 'headless' .claude/skills/grill-me/SKILL.md | - | cc:완료 | - |
...
| 4.13 | README 사용자 친화 개편 | README.md가 첫 화면, Quick Start, Codex CLI Setup, 작업별 workflow 메뉴, Troubleshooting을 사용자 행동 순서로 설명함 | grep -q '## 먼저 고를 것' README.md && grep -q '## Quick Start' README.md && grep -q '## Codex CLI Setup' README.md && grep -q '## 작업별 Workflow' README.md && grep -q '## Troubleshooting' README.md | - | cc:완료 | - |
