...
- `/harness-work` 구현은 반드시 대상 Task 전용 브랜치에서 한다. main/master에서 직접 구현하지 않는다. 브랜치가 없으면 `/branch-checkout`으로 `task/{task-id}-{short-slug}`를 새로 만들고, main에 이미 작업이 쌓였으면 `/rescue-from-main`으로 옮긴다. GitHub 연동 여부와 무관하게 적용한다.
