...
4. Task 전용 브랜치(`task/{task-id}-{short-slug}`) 생성·전환 — main/master 직접 구현 금지
5. `.harness/tasks/<task-key>/STATE.md` 갱신
6. TDD red evidence 확인 또는 예외 사유 기록
7. 구현
8. Acceptance와 관련 테스트를 fresh verification으로 실행
9. 리뷰
10. 통과 시 `tasks/index.json` 상태 변경과 `Plans.md` 재생성
