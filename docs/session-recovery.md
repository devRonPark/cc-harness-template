...
3. **`Plans.md`** — Task 상태 단일 출처. `cc:WIP` 행이 있으면 그게 재개 지점.
4. 그 다음은 **필요할 때만**, `.harness/CONTEXT_INDEX.md`로 골라서:
...
**Task 상태의 단일 출처는 Plans.md다.** `.harness/TASKS.md`에 Task 상태를
...
`Plans.md`·`git status`의 실제 상태와 어긋난 사례가 있었다(`.harness/LESSONS.md`
2026-07-03 항목). 요약은 참고만 하고, 판단은 항상 `Plans.md`와 `git status`
직접 확인으로 한다.
