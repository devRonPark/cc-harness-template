...
| `scripts/{validate_tasks,report_tasks,sync_plans}.py` | 포함 | Task JSON 검증·진행률 출력·읽기용 Plans.md snapshot 생성 |
...
# 3. 할 일 추가 (task-decomposer가 먼저 최소 단위로 분해 → tasks/index.json에 Task 작성, 필요 시 Plans.md snapshot 갱신)
/harness-plan
...
| `/harness-plan` | claude-code-harness 플러그인 | tasks/index.json Task 추가·관리 + Plans.md snapshot 갱신 |
