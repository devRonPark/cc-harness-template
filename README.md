...
**Claude Code Agent Harness Template** — Claude Code가 프로젝트 맥락을 안정적으로 유지하고,
계획·구현·검증·리뷰를 역할별로 나누고, 세션이 끊겨도 복구할 수 있게 만드는 repo-level 템플릿.
...
이 템플릿은 이 문제들을 **4개 Claude Code 플러그인 + `CLAUDE.md` 규약 + `.harness/` 상태 문서**
조합으로 해결한다. 새 프로젝트에 복사하고 이름만 바꾸면 바로 쓸 수 있다.
...
| `Plans.md` | 포함 | Task 상태 단일 출처 (`cc:TODO`/`cc:WIP`/`cc:완료` + DoD·Acceptance) |
| `BLUEPRINT.md` | 포함 | 플러그인·에이전트 협력 구조 전체 설명 (읽기용) |
| `agents/task-decomposer.md`, `agents/test-agent.md` | 포함 | 세분화 게이트·런타임 검증 절차 — 세션 Claude가 직접 수행하는 규약 문서 (서브에이전트 아님) |
...
| `.claude/commands/` | **미포함** | `/harness-*` 명령은 로컬 파일이 아니라 설치된 플러그인이 제공 (아래 [Commands](#claude-code-commands) 참고) |
| `scripts/verify-harness.sh` | **미포함** | 대체 수단: `harness doctor` (설치 상태) + Plans.md의 Task별 Acceptance 명령 (기능 검증) |
...
# 2. 내 프로젝트에 적용 (harness.toml·CLAUDE.md·Plans.md·.harness/ 등 전부 복사)
...
### Step 1 — `~/.claude/settings.json`에 플러그인 등록
...
Then check harness.toml, Plans.md, and BLUEPRINT.md.
...
Before making risky changes, update .harness/STATE.md.
After implementation, run the Acceptance command recorded for the task in Plans.md.
If an error occurs and you fix it, record the cause and prevention rule in
.harness/LOG.md and .harness/LESSONS.md.
```
...
```text
먼저 CLAUDE.md를 읽어줘.
그다음 harness.toml, Plans.md, BLUEPRINT.md를 확인해줘.
CLAUDE.md의 harness 규칙을 따라줘 — Task 작성 전 task-decomposer 세분화 게이트,
리뷰 전 test-agent 검증 게이트, .harness/ 상태 문서 규칙 전부 포함.
위험한 변경 전에는 .harness/STATE.md를 갱신해줘.
구현 후에는 Plans.md에 기록된 해당 Task의 Acceptance 명령을 실행해줘.
에러가 발생했고 해결했다면 원인과 재발 방지 규칙을 .harness/LOG.md와
.harness/LESSONS.md에 기록해줘.
```
...
**`Plans.md`**
Week 구조와 Task를 정의한다. DoD는 **검증 가능한 형태**로, Acceptance는 **CI가 직접 실행하는 명령어**로 쓴다.
...
4. `Plans.md`에 "기존 코드 파악" Week 0 Task를 추가한다 — DoD·Acceptance를
   명시해 [세분화 게이트](#구현-규칙-세분화-게이트)를 통과하게 쓴다.
...
3. **worker**에게 구현 위임 (ponytail 7단계 + VFF 검증 의무) — 진행 중 범위
   초과를 발견하면 즉시 멈추고 task-decomposer를 재호출해 하위 Task로 분리
...
6. 완료 반영: **GitHub 연동 시** PR 안에서 세션이 직접 `cc:완료`로 바꾸지
   ...
   **GitHub 미연동 시**는 세션이 Task 완료 시점에 Plans.md를 직접 갱신한다.
...
| `/harness-plan` | claude-code-harness 플러그인 | Plans.md Task 추가·관리 |
...
| `/harness-sync` | claude-code-harness 플러그인 | Plans.md ↔ 구현 상태 동기화 확인 |
...
- 세션 종료 시 `.harness/STATE.md` 갱신 여부 확인
- Acceptance 명령 미실행 상태에서 Task를 `cc:완료`로 표시하는 것 방지
...
1. `.harness/STATE.md` — 현재 상태 스냅샷
2. `.harness/LESSONS.md` — 최근 5개 항목 (재발 방지 기록)
3. `Plans.md` — Task 상태 단일 출처 (`cc:WIP` 행 확인)
4. 필요할 때만 `.harness/HANDOFF.md`(직전 세션 인수인계), `.harness/TASKS.md`(현재
   Task 체크리스트), `.harness/LOG.md`(에러 이력) — `.harness/CONTEXT_INDEX.md`가
   전체 인덱스다.
...
에러 기록은 `.harness/LOG.md`(append-only, 원문 그대로)와 `.harness/LESSONS.md`
(해결 후 재발 방지 요약)로 나뉜다 — 하나의 파일에 섞지 않는다.

- `.harness/LOG.md`: 시간 역순 아님, 위에서 아래로 추가만. 실패한 명령·에러
  메시지·조치를 있는 그대로 남긴다.
...
├── LICENSE                         # MIT
│
├── agents/                         # 절차 문서 — 수행 주체는 세션 Claude (서브에이전트 아님)
│   ├── task-decomposer.md          # 계획 세분화 + 구현 게이트 정의 (harness.toml [plan])
│   └── test-agent.md               # test-agent 정의 (Acceptance 실행 + 테스트 스위트)
│
├── templates/skeleton/             # init.sh가 복사하는 Plans.md·.harness/ 초기 상태 (dogfood 이력 없음)
│   ├── Plans.md
│   └── .harness/                   # 7종 골격 (아래 .harness/ 항목과 동일 구성)
│
├── .harness/                       # 세션 상태 문서 (Task 상태 단일 출처는 Plans.md, 여긴 세션 맥락만)
│   ├── STATE.md / HANDOFF.md / TASKS.md / LOG.md / LESSONS.md / CHECKPOINTS.md
│   └── CONTEXT_INDEX.md            # 파일 역할 인덱스 — 세션 재개 시 여기서 필요한 파일만 선별
│
├── .github/                        # GitHub 연동 시 사용 (harness.toml enabled = true)
│   ├── workflows/
│   │   ├── ci.yml                  # 빌드·테스트 CI + ci-ok 요약 잡(required check 이름 고정용)
│   │   ├── plans-guard.yml         # PR 시 header-check·wip-branch-check·plans-diff-check·
│   │   │                           #   depends-check·acceptance-check·granularity-check 실행
│   │   └── plans-complete.yml      # 머지 시 cc:WIP→cc:완료 자동 커밋 (push 실패 시 PR 폴백)
...
로드됐는지, Plans.md 해당 Task에 Acceptance가 명령어 형태로 적혀 있는지 먼저 확인한다.
...
- PR 전 `harness doctor` + 변경과 관련된 Plans.md Acceptance 명령을 실행한다.
- `CLAUDE.md`/`harness.toml`을 바꾸면 이유를 커밋 메시지나 Plans.md에 남긴다 —
...
| `docs/session-recovery.md` | `.harness/` 재개 절차 심화 (여러 프로젝트 동시 운영 시나리오 포함) |
| `docs/error-memory.md` | `LOG.md`/`LESSONS.md` 작성 규칙 + 실제 사례 모음 |
