...
| `Plans.md` | 포함 | 사람이 읽는 Task 로드맵. `tasks/index.json`에서 생성하며 직접 편집하지 않음 |
...
| `scripts/{validate_tasks,report_tasks,sync_plans}.py` | 포함 | Task JSON 검증·진행률 출력·Plans.md 동기화 |
...
**`tasks/index.json`**
Week 구조와 Task를 정의한다. DoD는 **검증 가능한 형태**로, Acceptance는 **CI가 직접 실행하는 명령어**로 쓴다. 수정 후 `python3 scripts/sync_plans.py`로 `Plans.md`를 재생성한다.
...
# 3. 할 일 추가 (task-decomposer가 먼저 최소 단위로 분해 → tasks/index.json에 Task 작성, Plans.md 동기화)
/harness-plan
...
# 6. tasks/index.json ↔ Plans.md ↔ 구현 동기화 확인
/harness-sync
```
...
| `/harness-plan` | claude-code-harness 플러그인 | tasks/index.json Task 추가·관리 + Plans.md 동기화 |
...
| `done` | `cc:완료` | 완료. **GitHub 연동**: `plans-complete.yml` 봇이 머지 후 자동 커밋. **미연동**: 세션이 직접 갱신 |
...
3. `tasks/index.json` — Task 상태 단일 출처 (`wip` Task 확인)
4. `Plans.md` — 사람이 읽는 동기화 산출물
5. 필요할 때만 `.harness/HANDOFF.md`(직전 세션 인수인계), `.harness/TASKS.md`(현재
...
├── Plans.md                        # 사람이 읽는 Task 로드맵 (tasks/index.json에서 생성)
...
│   └── sync_plans.py               # tasks/index.json → Plans.md 동기화
│
...
│
├── .harness/                       # 세션 상태 문서 (Task 상태 단일 출처는 tasks/index.json, 여긴 세션 맥락만)
...
│   │   └── plans-complete.yml      # 머지 시 wip→done 자동 커밋 + Plans.md 동기화
