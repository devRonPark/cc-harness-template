...
```text
AGENTS.md를 읽어줘.
python3 scripts/report_tasks.py로 현재 상태를 확인해줘.
...
```text
먼저 CLAUDE.md를 읽어줘.
그다음 python3 scripts/report_tasks.py로 Task 현황을 확인해줘.
harness.toml과 BLUEPRINT.md는 설정·구조 확인이 필요할 때만 읽어줘.
...
1. `python3 scripts/report_tasks.py` 요약과 대상 Task grep 블록 (`tasks/index.json` 전체 읽기 금지)
2. `.harness/tasks/<task-key>/STATE.md`
3. `.harness/tasks/<task-key>/RUN_REPORT.md`가 있으면 확인
4. `.harness/LESSONS.md` 최근 5개 항목만
5. 필요할 때만 `.harness/CONTEXT_INDEX.md`에서 추가 파일 선택 (`Plans.md`는 사람용 snapshot — 에이전트는 읽지 않음)
