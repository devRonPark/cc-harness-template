...
### 에이전트 3종 — `/harness-work` 실행 시 자동으로 협업한다
...
cc-harness-template/
│
├── harness.toml                    # 프로젝트 이름·안전 규칙 설정
├── CLAUDE.md                       # Claude Code가 항상 읽는 프로젝트 규칙
├── Plans.md                        # 할 일 목록 (cc:TODO / cc:WIP / cc:완료)
...
**`Plans.md`**  
Week 구조와 Task를 정의한다.  
DoD(완료 기준)는 반드시 **검증 가능한 형태**로 쓴다.

```markdown
# 좋은 DoD 예시
`npm run build` 에러 0, `dist/index.js` 존재
...
4. **reviewer**에게 검토 요청 (VFF v2 전체 — 판단 근거 명확하게)
5. 완료 시 Plans.md 마커를 `cc:WIP` → `cc:완료`로 자동 업데이트
